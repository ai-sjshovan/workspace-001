# Scene 008: Review And Level 5 File

status: working-canon
issue: 001
chapter: 001
sequence_position: 008
last_updated: 2026-06-04

## Purpose

Use the female overseer voice to make the interrogation feel like an evaluation before it turns accusatory.

## Image Brief

The TV screen turns on with the CoreTech logo. Miguel's employee record appears, then a redacted Level 5 review file. The female overseer praises his credentials before presenting the discrepancy.

## Final Art Links

## Panels

### Panel 001

**Image:**

The TV screen turns on with the CoreTech logo.

**Text:**

None.

**Dialogue:**

**Female Overseer voice:**

> Miguel Garcia.
> K.I.N. Cybersecurity Division.
> Eleven years, eight months with CoreTech.
> Current designation: Agent 410.
> Current clearance: Level 3.

**SFX:**

screen hum

### Panel 002

**Image:**

Miguel looks toward the screen.

**Text:**

None.

**Dialogue:**

**Miguel:**

> I want my supervisor.

**Female Overseer:**

> This review has supervisor authorization.

**SFX:**

None.

### Panel 003

**Image:**

Miguel's employee file appears. Photo. Commendations. Performance blocks. Everything clean and corporate.

**Text:**

None.

**Dialogue:**

**Female Overseer:**

> Your record is exceptional.
> Pattern recognition above division standard.
> Breach response above division standard.
> Containment judgment repeatedly commended.

**Miguel:**

> Th--thank you?

**SFX:**

None.

### Panel 004

**Image:**

Silence. A second file appears. Redacted. Marked Level 5 review.

**Text:**

None.

**Dialogue:**

**Female Overseer voice:**

> Four months ago, you were selected for temporary Level 5 breach review.
> You identified a hostile pattern before automated containment did.
> You recommended rotation of exposed authorization material.

**SFX:**

file open tone

### Panel 005

**Image:**

Miguel tenses slightly.

**Text:**

None.

**Dialogue:**

**Miguel:**

> That incident was sealed.

**Female Overseer:**

> Yes.

**SFX:**

None.

### Panel 006

**Image:**

Beat. The redacted file remains on screen.

**Text:**

None.

**Dialogue:**

**Female Overseer:**

> However, tonight, a related pattern entered your sandbox.
> You did not isolate it.
> You did not mirror it.
> The route remained open until power was cut.

**Miguel:**

> It was moving through live traffic. Killing the route would have lost the trace.

**SFX:**

None.

### Panel 007

**Image:**

The screen stays static. The voice continues.

**Text:**

None.

**Dialogue:**

**Female Overseer:**

> You delayed mirror sync.

**Miguel:**

> A mirror would have collapsed containment.

**Female Overseer:**

> You allowed external packets to remain attached to your session.

**Miguel:**

> For source confirmation.

**SFX:**

None.

### Panel 008

**Image:**

Silence. Miguel waits. His answer starts to feel weak.

**Text:**

None.

**Dialogue:**

**Female Overseer:**

> Who are you working for?

**Miguel:**

> Wha-

**Female Overseer, sharper:**

> Who are you working for?

**Miguel:**

> What is this?

**SFX:**

None.

## Continuity Notes

- This scene intentionally keeps the old `mirror sync` language because it is part of the current interrogation draft, not the opening-room containment scene.
- If the line feels too technical later, revise here only.

## Open Questions

- Whether `mirror sync` should be made simpler for non-technical readers.

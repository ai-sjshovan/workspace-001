...
class WayfinderRouteSmokeTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.stale_collected_at = (
            datetime.fromisoformat(cls.recent_collected_at.replace("Z", "+00:00")) - timedelta(days=45)
        ).isoformat()
        cls.older_opportunity_at = (
            datetime.fromisoformat(cls.recent_collected_at.replace("Z", "+00:00")) - timedelta(days=75)
        ).isoformat()
            ...
            insert_signals(
                ...
                [
                    Signal(
                        ...
                        collected_at=cls.recent_collected_at,
                    ),
                    Signal(
                        source=cls.source_name,
                        ...
                        collected_at=cls.stale_collected_at,
                    ),
                ],
            )
            insert_products(
            ...
            insert_opportunities(
                ...
                [
                    ...
                    Opportunity(
                        ...
                        collected_at=cls.older_opportunity_at,
                    ),
                ],
    ...
    def test_smoke_routes_and_source_routes(self) -> None:
        ...
        self.assertIn('name="freshness"', body)
        self.assertIn('name="min_score"', body)
        self.assertIn('name="max_score"', body)
        self.assertIn('name="opportunity_sort"', body)
        ...
        payload = json.loads(body)
        self.assertEqual(len(payload), 1)
        self.assertEqual(payload[0]["source_id"], "dashboard-smoke-2")
        ...
        detail_score = payload["opportunity_score"]

        status, body = self.fetch(f"/api/opportunities/{opportunity_id}/score")
        self.assertEqual(status, 200)
        payload = json.loads(body)
        self.assertEqual(payload["id"], opportunity_id)
        self.assertEqual(payload["fingerprint"], opportunity_fingerprint)
        self.assertEqual(payload["opportunity_score"], detail_score)
        self.assertEqual(payload["source"], self.source_name)
        self.assertEqual(payload["category"], "market-research")
        self.assertEqual(payload["score_breakdown"]["formula_version"], "deterministic-v1")
        self.assertIn("reference_time", payload["score_breakdown"])
        self.assertIn("inputs", payload["score_breakdown"])
        self.assertIn("weights", payload["score_breakdown"])
        ...
        self.assertIn("Adapter review summary", body)
        self.assertIn("Read-only adapter posture for oss-ledger", body)
        self.assertIn("This page does not run ingest, mutate source policy, or change adapter config.", body)
        self.assertIn("Review state", body)
        self.assertIn("Outstanding review flags", body)
        self.assertIn("Adapter health note", body)
        self.assertIn("Showing the latest source-linked sample rows captured in local storage.", body)
        self.assertIn("Sanitized config snapshot", body)
        ...
        self.assertEqual(status, 200)
        self.assertIn("Focused record selected: Wayfinder dashboard smoke signal.", body)
        ...
        status, body = self.fetch("/api/opportunities/missing-opportunity/score")
        self.assertEqual(status, 404)
        payload = json.loads(body)
        self.assertEqual(payload["error"], "opportunity_not_found")
        self.assertEqual(payload["requested_identifier"], "missing-opportunity")
